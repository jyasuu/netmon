...
pub fn render(snap: &Snapshot, scrapes: u64) -> String {
    ...
    macro_rules! g {
        ($name:expr, $val:expr) => { o.push_str(&format!("{} {}\n", $name, $val)); };
        ($name:expr, $labels:expr, $val:expr) => {
            o.push_str(&format!("{}{{{}}} {}\n", $name, $labels, $val));
        };
    }
    macro_rules! c {
        ($name:expr, $val:expr) => { o.push_str(&format!("{}_total {}\n", $name, $val)); };
        ($name:expr, $labels:expr, $val:expr) => {
    ...
    // ── scrape meta ──
    meta!("netmon_scrape_count", "Total Prometheus scrapes served", "counter");
    c!("netmon_scrape_count", scrapes);
    meta!("netmon_collection_timestamp_ms", "Unix ms of last data collection", "gauge");
    g!("netmon_collection_timestamp_ms", epoch_ms());

    // ── global connection stats ──
    let s = &snap.stats;
    meta!("netmon_connections_total",       "Total socket entries in /proc/net", "gauge");
    g!("netmon_connections_total",       s.total);
    meta!("netmon_connections_established", "Sockets in ESTABLISHED state", "gauge");
    g!("netmon_connections_established", s.established);
    meta!("netmon_connections_listen",      "Sockets in LISTEN state",      "gauge");
    g!("netmon_connections_listen",      s.listen);
    meta!("netmon_connections_time_wait",   "Sockets in TIME_WAIT state",   "gauge");
    g!("netmon_connections_time_wait",   s.time_wait);
    meta!("netmon_connections_close_wait",  "Sockets in CLOSE_WAIT state",  "gauge");
    g!("netmon_connections_close_wait",  s.close_wait);
    meta!("netmon_connections_syn_sent",    "Sockets in SYN_SENT state",    "gauge");
    ...
    meta!("netmon_rx_bytes_per_second", "Aggregate inbound throughput across all interfaces", "gauge");
    g!("netmon_rx_bytes_per_second", s.total_rx_bps);
    meta!("netmon_tx_bytes_per_second", "Aggregate outbound throughput across all interfaces", "gauge");
    g!("netmon_tx_bytes_per_second", s.total_tx_bps);

    // ── per-state connection count ──
    let mut state_map: HashMap<String, usize> = HashMap::new();
    for c in &snap.connections { *state_map.entry(c.state.clone()).or_insert(0) += 1; }
    meta!("netmon_connection_state_count", "Number of connections by state", "gauge");
    let mut states: Vec<_> = state_map.iter().collect();
    states.sort_by_key(|(k,_)| k.as_str());
    for (state, count) in states {
        g!("netmon_connection_state_count", format!("state=\"{}\"", esc(state)), count);
    }

    // ── per-protocol count ──
    let mut proto_map: HashMap<String, usize> = HashMap::new();
    for c in &snap.connections { *proto_map.entry(c.protocol.as_str().to_string()).or_insert(0) += 1; }
    meta!("netmon_connection_protocol_count", "Number of connections by protocol", "gauge");
    for (proto, count) in &proto_map {
        g!("netmon_connection_protocol_count", format!("protocol=\"{}\"", esc(proto)), count);
    }

    // ── listen ports ──
    meta!("netmon_listen_port", "1 if the port is being listened on, labelled by protocol+port", "gauge");
    let mut ports: Vec<_> = snap.connections.iter()
        .filter(|c| c.state == "LISTEN")
        .map(|c| (c.protocol.as_str(), c.local_port))
        .collect::<std::collections::HashSet<_>>().into_iter().collect();
    ports.sort();
    for (proto, port) in ports {
        g!("netmon_listen_port", format!("protocol=\"{proto}\",port=\"{port}\""), 1);
    }

    // ── interface metrics ──
    macro_rules! iface_counter {
        ($name:literal, $help:literal, $field:ident) => {
            meta!(concat!("netmon_interface_", $name, "_total"), $help, "counter");
            for i in &snap.ifaces {
                c!(concat!("netmon_interface_", $name),
                   format!("interface=\"{}\"", esc(&i.name)), i.$field);
            }
        };
    }
    macro_rules! iface_gauge {
        ($name:literal, $help:literal, $field:ident) => {
            ...
            for i in &snap.ifaces {
                g!(concat!("netmon_interface_", $name),
                   format!("interface=\"{}\"", esc(&i.name)), i.$field);
            }
        };
    }
    ...
    iface_gauge!("rx_bytes_per_second", "Current RX rate bytes/s", rx_rate_bps);
    iface_gauge!("tx_bytes_per_second", "Current TX rate bytes/s", tx_rate_bps);
    ...
    proc_metric!("connection_count",     "Active connections per process",              "gauge",   connections);
    proc_metric!("rx_bytes",             "Cumulative read bytes (from /proc/PID/io)",   "counter", rx_bytes);
    proc_metric!("tx_bytes",             "Cumulative written bytes",                    "counter", tx_bytes);
    proc_metric!("rx_bytes_per_second",  "Current RX rate per process",                "gauge",   rx_bytes_delta);
    proc_metric!("tx_bytes_per_second",  "Current TX rate per process",                "gauge",   tx_bytes_delta);
    proc_metric!("memory_bytes",         "RSS memory bytes",                            "gauge",   mem_bytes);

    // cpu is f32 so handle separately
    meta!("netmon_process_cpu_percent", "CPU usage % per process", "gauge");
    for p in &snap.procs {
        let lbl = format!("pid=\"{}\",process=\"{}\"", p.pid, esc(&p.name));
        o.push_str(&format!("netmon_process_cpu_percent{{{}}} {:.2}\n", lbl, p.cpu_pct));
    }

    // ── per-process full cmdline as info metric ──
    meta!("netmon_process_info", "Process info (cmdline label), value always 1", "gauge");
    for p in &snap.procs {
        let cmd = p.cmdline.replace('"', "\\\"");
        let lbl = format!("pid=\"{}\",process=\"{}\",cmdline=\"{}\"", p.pid, esc(&p.name), esc(&cmd));
...
fn index_html(port: u16) -> String {
    format!(r#"<!DOCTYPE html><html><head><title>netmon exporter</title>
...
pub fn run(port: u16, interval_ms: u64, running: Arc<AtomicBool>) -> Result<()> {
    ...
    eprintln!("  Interval: {:.1}s  │  Ctrl-C to stop", interval_ms as f64/1000.0);
    ...
    // ── collector thread ──
    {
        ...
        std::thread::spawn(move || {
            ...
            while run_c.load(Ordering::SeqCst) {
                ...
                eprintln!("  [collect] conns:{} procs:{} rx:{:.1}KB/s tx:{:.1}KB/s",
                    s.stats.total, s.procs.len(),
                    s.stats.total_rx_bps as f64/1024.0,
                    s.stats.total_tx_bps as f64/1024.0);
                if let Ok(mut g) = snap_w.lock() { *g = s; }
                let ticks = (interval.as_millis()/100).max(1) as u64;
                for _ in 0..ticks {
                    if !run_c.load(Ordering::SeqCst) { break; }
            ...
            Ok((mut stream, addr)) => {
                ...
                std::thread::spawn(move || {
                    ...
                    match read_path(&mut stream).as_deref() {
                        ...
                        Some("/health") => {
                            respond(&mut stream, "200 OK", "application/json", "{\"status\":\"ok\"}");
                        }
                        Some("/") | Some("/index.html") => {
                            respond(&mut stream, "200 OK", "text/html; charset=utf-8", &index_html(port_c));
                        }
                        _ => {
