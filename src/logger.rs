...
fn fb(b: u64) -> String {
    ...
    else if b < 1_073_741_824 { format!("{:.2} MB", b as f64/1_048_576.0) }
    else                      { format!("{:.3} GB", b as f64/1_073_741_824.0) }
}

fn fr(b: u64) -> String {
    if b == 0             { return "0 B/s".into(); }
    if b < 1024           { format!("{b} B/s") }
    else if b < 1_048_576 { format!("{:.2} KB/s", b as f64/1024.0) }
    else                  { format!("{:.2} MB/s", b as f64/1_048_576.0) }
}

fn bar(val: u64, max: u64, w: usize) -> String {
    if max == 0 { return "░".repeat(w); }
    let f = ((val as f64 / max as f64) * w as f64) as usize;
    format!("{}{}", "█".repeat(f.min(w)), "░".repeat(w.saturating_sub(f)))
}
...
fn wrap(s: &str, w: usize) -> Vec<String> {
    if w == 0 || s.is_empty() { return vec![s.to_string()]; }
    s.as_bytes().chunks(w).map(|c| String::from_utf8_lossy(c).into()).collect()
}
...
fn write_report(w: &mut impl Write, snap: &Snapshot, n: u64, iv: f64) -> Result<()> {
    ...
    writeln!(w, " │  RX: {:>16}   TX: {:>16}",
        fr(s.total_rx_bps), fr(s.total_tx_bps))?;
    writeln!(w, " └{}", "─".repeat(W-1))?;

    // ── Interfaces ──────────────────────────────────────────────────────────
    writeln!(w, "\n ┌─[ NETWORK INTERFACES  count:{} ]{}", snap.ifaces.len(), "─".repeat(W.saturating_sub(33)))?;
    writeln!(w, " │  {:<14}  {:>13}  {:>13}  {:>13}  {:>13}  {:>9}  {:>9}",
        "INTERFACE", "RX TOTAL", "TX TOTAL", "RX/s", "TX/s", "RX ERR", "TX ERR")?;
    writeln!(w, " │  {}", "·".repeat(W-4))?;

    let max_r = snap.ifaces.iter().map(|i| i.rx_rate_bps.max(i.tx_rate_bps)).max().unwrap_or(1).max(1);
    for i in &snap.ifaces {
        writeln!(w, " │  {:<14}  {:>13}  {:>13}  {:>13}  {:>13}  {:>9}  {:>9}",
            i.name, fb(i.rx_bytes), fb(i.tx_bytes), fr(i.rx_rate_bps), fr(i.tx_rate_bps),
            i.rx_errors, i.tx_errors)?;
        writeln!(w, " │    RX [{}] {:>13}", bar(i.rx_rate_bps, max_r, 42), fr(i.rx_rate_bps))?;
        writeln!(w, " │    TX [{}] {:>13}", bar(i.tx_rate_bps, max_r, 42), fr(i.tx_rate_bps))?;
    }
    writeln!(w, " └{}", "─".repeat(W-1))?;

    // ── Per-process traffic ─────────────────────────────────────────────────
    writeln!(w, "\n ┌─[ PROCESS TRAFFIC  active:{} ]{}", snap.procs.len(), "─".repeat(W.saturating_sub(31)))?;
    writeln!(w, " │  {:<7}  {:<22}  {:>5}  {:>7}  {:>9}  {:>14}  {:>14}  {:>12}  {:>12}",
        "PID", "PROCESS", "CONNS", "CPU%", "MEM",
        "RX/s", "TX/s", "RX TOTAL", "TX TOTAL")?;
    writeln!(w, " │  {}", "·".repeat(W-4))?;

    let max_p = snap.procs.iter().map(|p| p.rx_bytes_delta.max(p.tx_bytes_delta)).max().unwrap_or(1).max(1);
    for p in &snap.procs {
        writeln!(w, " │  {:<7}  {:<22}  {:>5}  {:>6.1}%  {:>9}  {:>14}  {:>14}  {:>12}  {:>12}",
            p.pid, trunc(&p.name, 22), p.connections,
            p.cpu_pct, fb(p.mem_bytes),
            fr(p.rx_bytes_delta), fr(p.tx_bytes_delta),
            fb(p.rx_bytes), fb(p.tx_bytes))?;
        writeln!(w, " │     RX [{}] {:>13}", bar(p.rx_bytes_delta, max_p, 38), fr(p.rx_bytes_delta))?;
        writeln!(w, " │     TX [{}] {:>13}", bar(p.tx_bytes_delta, max_p, 38), fr(p.tx_bytes_delta))?;
        // Full command line, wrapped
        if !p.cmdline.is_empty() {
            for (i, chunk) in wrap(&p.cmdline, W-18).iter().enumerate() {
                if i==0 { writeln!(w, " │     cmdline: {chunk}")?; }
    ...
    writeln!(w, "\n ┌─[ ALL CONNECTIONS  total:{} ]{}", snap.connections.len(), "─".repeat(W.saturating_sub(30)))?;
...
pub fn run(output_path: &str, interval_ms: u64, running: Arc<AtomicBool>) -> Result<()> {
    ...
    eprintln!("netmon log-mode  →  '{output_path}'  │  interval={interval_s:.1}s  │  Ctrl-C to stop");
    ...
    // Write file header
    {
        ...
        writeln!(bw, "# started  : {}", Local::now().format("%Y-%m-%d %H:%M:%S %Z"))?;
    ...
    while running.load(Ordering::SeqCst) {
        ...
        eprintln!("[#{sample:>4}] conns:{:>5}  procs:{:>4}  rx:{:>14}  tx:{:>14}",
            snap.stats.total, snap.procs.len(),
            fr(snap.stats.total_rx_bps), fr(snap.stats.total_tx_bps));
